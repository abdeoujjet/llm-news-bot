...
def selector_noticias_relevantes(news_list):
    ...
    def procesar_noticia(noticia):
        resumen = resumir_web(noticia['url'], modelo_resumen_noticia)
        noticia['resumen_breve'] = resumen
        return noticia

    # Procesamos las noticias en paralelo (hasta 5 a la vez)
    with ThreadPoolExecutor(max_workers=5) as executor:
        respuesta_modelo['noticias_relevantes'] = list(executor.map(procesar_noticia, respuesta_modelo['noticias_relevantes']))
